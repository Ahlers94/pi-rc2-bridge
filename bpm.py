...
def run_tap_tempo(bpm: float, tap_pin: int, pulse_ms: int, taps: int | None) -> None:
    if bpm <= 0:
...
def main() -> None:
    ...
    if args.stop:
        run_stop(args.gpio_stop, args.pulse_ms)
    elif args.bpm is not None:
        run_tap_tempo(args.bpm, args.gpio_tap, args.pulse_ms, args.taps)
    else:
        parser.print_help()
        sys.exit(1)
